			...
			<div class="col-md-6 col-md-offset-3">
				<table class="table table-bordered table-hover table-striped">
					...
					<tr>
						...
						<th>时间</th>
						<th>大小</th>
						<th class="text-center"><c:if
								test="${fn:contains(authOfManageFiles,10101)}">
								<a href="UploadFile" class="btn btn-primary btn-xs"
									role="button">上传</a>
							</c:if></th>
					</tr>
					<c:forEach items="${reqResult}" var="doc">
						<tr>
							<td><c:if test="${fn:contains(authOfManageFiles,10102)}">
									<a href="DownloadFile?id=${doc.id}">${doc.fileName}</a>
								</c:if> <c:if test="${!fn:contains(authOfManageFiles,10102)}">
				${doc.fileName}</c:if></td>
							<td><fmt:formatDate value="${doc.dateTime}"
									pattern="yyyy-MM-dd HH:mm:ss" /></td>
							<td>${doc.fileSize}</td>
							<td class="text-center"><c:if
									test="${fn:contains(authOfManageFiles,10103)}">
									<a href="DelFile?id=${doc.id}" class="btn btn-primary btn-xs"
										role="button">删除</a>
								</c:if></td>
						</tr>
					</c:forEach>
