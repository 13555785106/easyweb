...
<body>
	<form method="post">
		<!--
	 此处设置不需要进行trim的字段，如果没有设置，表单数据将全部进行trim。
	 每个字段使用方括号括起来，各个字段之间使用逗号分隔。
	 -->
		<input type="hidden" name="fieldNotNeedTrim" value="[passwd],[confirmPasswd]" /> 
		<input type="hidden" name="arraySeparator" value="," /> 
		<input type="hidden" name="arrayFields" value="[hobbies],[authorities]" /> <input type="hidden" name="id"
			value="${reqParams.id}" />
		<table align="center">
			<caption>修改账号</caption>
			<tr>
			...
			<tr>
				...
				<td><input type="text" name="account" value="${reqParams.account}" /></td>
				<td><span> ${paramErrors.account[0]}</span></td>
			</tr>
			<tr>
				<th>密码：<span style="color: red; font-weight: bolder;">*</span></th>
				<td><input type="text" name="passwd" value="${reqParams.passwd}" /></td>
				<td><span>${paramErrors.passwd[0]}</span></td>
			</tr>
			<tr>
				<th>确认密码：<span style="color: red; font-weight: bolder;">*</span></th>
				<td><input type="text" name="confirmPasswd" value="${reqParams.confirmPasswd}" /></td>
				<td><span>${paramErrors.confirmPasswd[0]}</span></td>
			</tr>
			<tr>
				<th>生日：</th>
				<td><input type="text" name="birthday" value="${reqParams.birthday}" size="12" /></td>
				<td><span>${paramErrors.birthday[0]}</span></td>
			</tr>
			<tr>
				<th>性别：<span style="color: red; font-weight: bolder;">*</span></th>
				<td><c:set var="defaultSex" value="${reqParams.sex}" /> <c:if test="${empty defaultSex}">
			...
			<tr>
				<th>工资：</th>
				<td><fmt:formatNumber value="${reqParams.salary}" var="salary" pattern="0.00" /> <input type="text" name="salary" value="${salary}" size="6" /></td>
			...
			<tr>
				...
				<c:forEach items="${authTypes}" var="authTypeItem">
					${authTypeItem.key.name}<br/>
					    <c:if test="${authTypeItem.key.mode==0}">
					    	<input type="radio" name="authorities" value="99999" checked />无
					    </c:if>
